...
import (
	...
	"github.com/huimingz/wechatgo/log"
...
func NewWechatClient(corpid, corpSecret string, agentId int, httpClient *http.Client,
	expiresIn time.Duration, mutex *sync.Mutex, storage_ storage.Storage, log_ log.Logger) *WechatClient {
	client := WechatClient{}
...
func (client *WechatClient) FetchAccessToken() error {
	...
	if accessToken.ErrCode == 0 && accessToken.AccessToken != "" {
		...
		return NewWXMsgError(accessToken.ErrCode, accessToken.ErrMsg)
	}
	return nil
}
...
func (client WechatClient) respHandler(resp *http.Response, errmsg WxMsgInterface, out interface{}) error {
	defer resp.Body.Close()
...
func (client *WechatClient) Get(url_ string, values url.Values, errmsg WxMsgInterface, out interface{}) error {
...
func (client *WechatClient) AdvPost(url_, contentType string, values url.Values, data interface{}, errmsg WxMsgInterface, out interface{}) error {
	values, err := client.valuesTokenCompletion(values)
	if err != nil {
		return err
	}
...
func (client WechatClient) Post(url_ string, values url.Values, data interface{}, errmsg WxMsgInterface, out interface{}) error {
