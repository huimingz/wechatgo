...
func TestWechatUser_UserId2OpenId(t *testing.T) {
	...
	if openId != testdata.TestConf.OpenId {
		t.Errorf("WechatUser.UserId2OpenId() error = '返回的openid与期望不符'")
	}
}
...
func TestWechatUser_DeleteUser(t *testing.T) {
	...
	err = wechatUser.DeleteUser("example_xxx")
	if err != nil {
		if v, ok := err.(*client.WXMsgError); ok {
			if v.ErrCode != 60111 {
...
func init() {
	...
	var wechatClient = client.NewWechatClient(conf.CorpId, conf.UserSecret, conf.AgentId, nil, 0, nil, nil, nil)
