...
func NewWechatAppManage(client *client.WechatClient) *WechatAppManage {
...
func (w WechatAppManage) GetAllApp() ([]AppIntro, error) {
	agentList := struct {
		AgentList []AppIntro `json:"agentlist"`
	}{}
	err := w.Client.Get(urlGetAllApp, nil, nil, &agentList)
...
//
// 参考文档：https://work.weixin.qq.com/api/doc#90000/90135/90227
func (w WechatAppManage) GetApp(agentId int) (*AppDetail, error) {
	values := url.Values{}
	if agentId == 0 {
	...
	appDetail := AppDetail{}
	err := w.Client.Get(urlGetApp, values, nil, &appDetail)
...
func (w WechatAppManage) CreateApp(appInfo AppInfo) error {
	return w.Client.Post(urlCreateApp, nil, appInfo, nil, nil)
}

// 创建菜单
//
// 参考资料：https://work.weixin.qq.com/api/doc#90000/90135/90230
func (w WechatAppManage) CreateMenu(menu Menu, agentId int) error {
	values := url.Values{}
	if agentId == 0 {
		agentId = w.Client.AgentId
	}
	...
	return w.Client.Post(urlCreateMenu, values, menu, nil, nil)
}

// 获取菜单
//
// 参考文档：https://work.weixin.qq.com/api/doc#90000/90135/90232
func (w WechatAppManage) GetMenu(agentId int) (*Menu, error) {
	values := url.Values{}
	if agentId == 0 {
		agentId = w.Client.AgentId
	}
	...
	menu := Menu{}
	err := w.Client.Get(urlGetMenu, values, nil, &menu)
	return &menu, err
}
...
func (w WechatAppManage) DeleteMenu(agentId int) error {
	values := url.Values{}
	if agentId == 0 {
		agentId = w.Client.AgentId
	}
	...
	return w.Client.Get(urlDelMenu, values, nil, nil)
}
